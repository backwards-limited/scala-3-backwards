...
class StateApplicativeSuite extends ScalaCheckSuite {
  ...
  property("State Applicative pure") {
    assertEquals(
      Applicative[[A] =>> State[String, A]].pure(5).run("hello"),
      "hello" -> 5
    )

    assertEquals(
      Applicative[[A] =>> State[List[String], A]].pure(5).run(List("hello")),
      List("hello") -> 5
    )
  }
  ...
  property("State Applicative pure syntax") {
    ...
    assertEquals(
      5.pure[[A] =>> State[String, A]].run("hello"),
      "hello" -> 5
    )

    assertEquals(
      5.pure[[A] =>> State[List[String], A]].run(List("hello")),
      List("hello") -> 5
    )
  }
  ...
  property("State (String accumulator) Applicative ap") {
    ...
    val addFirstPartiallyApplied: State[String, Int => Int => Int] =
      Functor[[A] =>> State[String, A]].fmap(State(s => s"${s}a" -> 5))(add)

    val addSecondPartiallyApplied: State[String, Int => Int] =
      Applicative[[A] =>> State[String, A]].ap(addFirstPartiallyApplied)(State(s => s"${s}b" -> 10))

    val addThirdAndLastPartiallyApplied: State[String, Int] =
      Applicative[[A] =>> State[String, A]].ap(addSecondPartiallyApplied)(State(s => s"${s}c" -> 20))
  ...
  property("State (List accumulator) Applicative ap") {
    ...
    val addFirstPartiallyApplied: State[List[String], Int => Int => Int] =
      Functor[[A] =>> State[List[String], A]].fmap(State(s => (s :+ "a") -> 5))(add)

    val addSecondPartiallyApplied: State[List[String], Int => Int] =
      Applicative[[A] =>> State[List[String], A]].ap(addFirstPartiallyApplied)(State(s => (s :+ "b") -> 10))

    val addThirdAndLastPartiallyApplied: State[List[String], Int] =
      Applicative[[A] =>> State[List[String], A]].ap(addSecondPartiallyApplied)(State(s => (s :+ "c") -> 20))
