...
class DisjunctionMonadTransformerIdSuite extends ScalaCheckSuite {
  ...
  property("DisjunctionT Functor and Applicative syntax") {
    ...
    assertEquals(
      DisjunctionT(Id(10.right[String])) `<$>` (x => (y: Int) => x + y) <*> DisjunctionT(Id(1.right)),
      DisjunctionT(Id(11.right))
    )

    assertEquals(
      DisjunctionT(Id("whoops".left[Int])) `<$>` (x => (y: Int) => x + y) <*> DisjunctionT(Id(1.right)),
      DisjunctionT(Id("whoops".left))
    )

    assertEquals(
      DisjunctionT(Id(10.right[String])) `<$>` (x => (y: Int) => x + y) <*> DisjunctionT(Id("whoops".left)),
      DisjunctionT(Id("whoops".left))
    )
    ...
    assertEquals(
      DisjunctionT(Id(10.right[String])) `<$>` add <*> DisjunctionT(Id("whoops".left)),
      DisjunctionT(Id("whoops".left))
    )
  }
