...
class MaybeMonadTransformerStateSuite extends ScalaCheckSuite {
  ...
  property("MaybeT Monad syntax") {
    ...
    assertEquals(
      10.pure[MaybeT[State[List[String], *], *]].flatMap(_ => MaybeT(State((_: List[String]) -> nothing[Int]))).value.run(List("foo")),
      List("foo") -> nothing[Int]
    )

    assertEquals(
      MaybeT(State((_: List[String]) -> Just(1))).flatMap(a => MaybeT(State((_: List[String]) -> Just(a + 1)))).value.run(List("foo")),
      List("foo") -> Just(2)
    )

    assertEquals(
      MaybeT(State((_: List[String]) ++ List("bar") -> Just(1))).flatMap(a => MaybeT(State((_: List[String]) ++ List("baz") -> Just(a + 1)))).value.run(List("foo")),
      List("foo", "bar", "baz") -> Just(2)
    )
  }
  ...
  property("MaybeT Applicative syntax") {
    ...
    assertEquals(
      ((x: Int) => x + 1).pure[MaybeT[State[List[String], *], *]].ap(MaybeT(State((_: List[String]) -> Nothing[Int]))).value.run(List("foo")),
      List("foo") -> Nothing[Int]
    )

    assertEquals(
      MaybeT(State((_: List[String]) -> Just((x: Int) => x + 1))).ap(MaybeT(State((_: List[String]) -> Just(1)))).value.run(List("foo")),
      List("foo") -> Just(2)
    )

    assertEquals(
      MaybeT(State((_: List[String]) ++ List("bar") -> Just((x: Int) => x + 1))).ap(MaybeT(State((_: List[String]) ++ List("baz") -> Just(1)))).value.run(List("foo")),
      List("foo", "bar", "baz") -> Just(2)
    )
  }
  ...
  property("MaybeT Functor and Applicative syntax") {
    ...
    assertEquals(
      (MaybeT(State((_: String) + "bar" -> 10.just)) `<$>` (x => (y: Int) => x + y) <*> MaybeT(State((_: String) + "baz" -> 1.just))).value.run("foo"),
      "foobarbaz" -> 11.just
    )

    assertEquals(
      (MaybeT(State((_: List[String]) ++ List("bar") -> nothing[Int])) `<$>` (x => (y: Int) => x + y) <*> MaybeT(State((_: List[String]) ++ List("baz") -> 1.just))).value.run(List("foo")),
      List("foo", "bar", "baz") -> nothing[Int]
    )

    assertEquals(
      (MaybeT(State((_: List[String]) ++List("bar") -> Just(1))) `<$>` (x => (y: Int) => x + y) <*> MaybeT(State((_: List[String]) ++ List("baz") -> nothing[Int]))).value.run(List("foo")),
      List("foo", "bar", "baz") -> nothing[Int]
    )
    ...
    assertEquals(
      (MaybeT(State((_: List[String]) ++ List("bar") -> 10.just)) `<$>` add <*> MaybeT(State((_: List[String]) ++ List("baz") -> nothing[Int]))).value.run(List("foo")),
      List("foo", "bar", "baz") -> nothing[Int]
    )

    assertEquals(
      (MaybeT(State((_: List[String]) ++ List("bar") -> 10.just)) `<$>` add <*> MaybeT(State((_: List[String]) ++ List("baz") -> 1.just))).value.run(List("foo")),
  ...
  property("MaybeT Functor and Applicative function syntax") {
    ...
    assertEquals(
      (((x: Int) => (y: Int) => x + y) `<$>` MaybeT(State((_: String) + "bar" -> 10.just)) <*> MaybeT(State((_: String) + "baz" -> 1.just))).value.run("foo"),
      "foobarbaz" -> 11.just
    )

    assertEquals(
      (((x: Int) => (y: Int) => x + y) `<$>` MaybeT(State((_: List[String]) ++ List("bar") -> nothing[Int])) <*> MaybeT(State((_: List[String]) ++ List("baz") -> 1.just))).value.run(List("foo")),
      List("foo", "bar", "baz") -> nothing[Int]
    )

    assertEquals(
      (((x: Int) => (y: Int) => x + y) `<$>` MaybeT(State((_: List[String]) ++ List("bar") -> 10.just)) <*> MaybeT(State((_: List[String]) ++ List("baz") -> nothing[Int]))).value.run(List("foo")),
      List("foo", "bar", "baz") -> nothing[Int]
    )
    ...
    assertEquals(
      (add `<$>` MaybeT(State((_: List[String]) ++ List("bar") -> 10.just)) <*> MaybeT(State((_: List[String]) ++ List("baz") -> 1.just))).value.run(List("foo")),
      List("foo", "bar", "baz") -> 11.just
    )

    assertEquals(
      (add `<$>` MaybeT(State((_: List[String]) ++ List("bar") -> 10.just)) <*> MaybeT(State((_: List[String]) ++ List("baz") -> nothing[Int]))).value.run(List("foo")),
      List("foo", "bar", "baz") -> nothing[Int]
    )
  }
